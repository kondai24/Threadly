...
metadata:
  ...
  openclaw:
    ...
    install:
      - kind: go
        package: golang.org/x/vuln/cmd/govulncheck@v1.6.0
        bins: [govulncheck]
allowed-tools: Read Edit Write Glob Grep Bash(go:*) Bash(golangci-lint:*) Bash(govulncheck:*) AskUserQuestion
---
...
**Orchestration mode:** Use deep reasoning for a full-codebase security audit. Review vulnerability domains sequentially in the current workspace unless the user explicitly requests approved delegation.
...
- **Audit mode** — full codebase security scan. Review these vulnerability domains sequentially: (1) injection patterns, (2) cryptography and secrets, (3) web security and headers, (4) authentication and authorization, (5) concurrency safety and dependency vulnerabilities. Aggregate findings, score with DREAD, and report by severity.
- **Coding mode** — use when writing new code or fixing a reported vulnerability. Follow the skill's sequential guidance and keep changes in the current workspace unless the user explicitly requests an approved isolated workflow.
...
go get -tool github.com/securego/gosec/v2/cmd/gosec@v2.28.0
go tool gosec ./...
