...
**Orchestration mode:** Audit error handling sequentially in the current workspace and consolidate the five categories (creation, wrapping, single-handling rule, panic/recover, structured logging). Do not use automatic delegation.

**Modes:**

- **Coding mode** — writing new error handling code. Follow the best practices sequentially and inspect adjacent code in the current workspace for violations (swallowed errors, log-and-return pairs) without automatic delegation.
...
- **Audit mode** — auditing existing error handling across a codebase. Review each independent category sequentially (creation, wrapping, single-handling rule, panic/recover, structured logging).
...
## Auditing Error Handling Safely

Audit error handling sequentially in the current workspace by default. Do not delegate work automatically; only use an explicitly approved delegation workflow when the user requests it and the host confirms the required capabilities.

- Error creation — validate `errors.New`/`fmt.Errorf` usage, low-cardinality messages, custom types
- Error wrapping — audit `%w` vs `%v`, verify `errors.Is`/`errors.As` patterns
- Single handling rule — find log-and-return violations, swallowed errors, discarded errors (`_`)
- Panic/recover — audit `panic` usage, verify recovery at goroutine boundaries
- Structured logging — verify `slog` usage at error sites, check for PII in error messages
