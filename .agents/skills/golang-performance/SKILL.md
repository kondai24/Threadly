...
metadata:
  ...
  openclaw:
    ...
    install:
      - kind: go
        package: golang.org/x/perf/cmd/benchstat@v0.0.0-20260709024250-82a0b07e230d
        bins: [benchstat]
allowed-tools: Read Edit Write Glob Grep Bash(go:*) Bash(golangci-lint:*) Bash(benchstat:*) Bash(fieldalignment:*) Bash(staticcheck:*) Bash(fgprof:*) Bash(perf:*) AskUserQuestion
---
...
**Orchestration mode:** Perform a broad architectural performance review sequentially across allocation and memory layout, I/O and concurrency, and algorithmic complexity/caching.

**Modes:**

- **Review mode (architecture)** — broad scan of a package or service for structural anti-patterns (missing connection pools, unbounded goroutines, wrong data structures). Review allocation/memory layout, I/O/concurrency, and algorithmic complexity/caching sequentially.
- **Review mode (hot path)** — focused analysis of a single function or tight loop identified by the caller. Work sequentially.
...
- benchstat: `go install golang.org/x/perf/cmd/benchstat@v0.0.0-20260709024250-82a0b07e230d`
...
When multiple candidate optimizations compete for the same bottleneck, implement and measure each sequentially in the current workspace. See `samber/cc-skills-golang@golang-benchmark` for the serial-measurement caveat (concurrent benchmark runs on shared CPU contaminate results).
