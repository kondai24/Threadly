...
**Orchestration mode:** Adopt linting on a legacy codebase sequentially across auto-fix, security linters, error handling, style/formatting, and code quality. Do not use automatic multi-stage delegation.
...
- **Interpret/fix mode** — reading lint output, suppressing warnings, fixing issues on existing code: start from "Interpreting Output" and "Suppressing Lint Warnings"; keep large-scale cleanup sequential in the current workspace.
...
## Cleaning Up Legacy Codebases Safely

When adopting linting on a legacy codebase, fix independent linter categories sequentially in the current workspace. Do not delegate or create worktrees automatically; only do so after explicit user approval and host confirmation.

- Auto-fixable issues: run `golangci-lint run --fix ./...` after reviewing the planned changes
- Security linter findings: bodyclose, sqlclosecheck, gosec
- Error handling issues: errcheck, nilerr, wrapcheck
- Style and formatting: gofumpt, goimports, revive
- Code quality: gocritic, unused, ineffassign
