...
**Orchestration mode:** Review code style sequentially in the current workspace. Do not use automatic multi-stage delegation; only use an explicitly approved delegated workflow when the user requests it.
...
## Reviewing Large Codebases Safely

Review large codebases sequentially in the current workspace by default. Do not delegate work or create worktrees automatically; only do so after the user explicitly requests it and the host confirms the required capabilities.
