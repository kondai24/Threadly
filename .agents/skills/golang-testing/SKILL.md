...
metadata:
  ...
  openclaw:
    ...
    install:
      - kind: go
        package: github.com/cweill/gotests/gotests@v1.9.0
        bins: [gotests]
allowed-tools: Read Edit Write Glob Grep Bash(go:*) Bash(golangci-lint:*) Bash(gotests:*) AskUserQuestion
---
...
**Orchestration mode:** Audit a large test suite sequentially in the current workspace across unit quality, integration isolation, and goroutine/race issues, then merge the findings into one gap report.
...
- **Audit mode** — auditing an existing test suite for gaps, flakiness, or bad patterns (order-dependent tests, missing `t.Parallel()`, implementation-detail coupling). Review unit quality, integration isolation/build tags, and goroutine leaks/race conditions sequentially.
...
- gotests: `go install github.com/cweill/gotests/gotests@v1.9.0`
