...
**Orchestration mode:** Use deep reasoning when refactoring a large coupled codebase toward dependency injection. Keep the review sequential in the current workspace unless the user explicitly requests approved delegation.
...
- **Refactor mode** (existing coupled code): inspect global variables and `init()` service setup, map concrete type dependencies that should become interfaces, and locate service-locator anti-patterns (container passed as argument); then consolidate findings and propose a migration plan.
