...
metadata:
  ...
  openclaw:
    ...
    install:
      - kind: go
        package: golang.org/x/tools/gopls@v0.42.0
        bins: [gopls]
      - kind: go
        package: golang.org/x/perf/cmd/benchstat@v0.0.0-20260709024250-82a0b07e230d
        bins: [benchstat]
    skill-library-version: "0.20.0"
allowed-tools: Read Edit Write Glob Grep Bash(go:*) Bash(golangci-lint:*) Bash(gopls:*) Bash(benchstat:*) LSP mcp__gopls__* AskUserQuestion
---
...
- **Execute mode** (human-in-the-loop) — one atomic change at a time in the current workspace, with a branch and PR only when the user explicitly requests that workflow. Keep each result small and reviewable. See [workflow.md](references/workflow.md).
