...
2. For each atomic change in the inventory, in the order established in Step 2, execute it directly in the current workspace after the planned human review checkpoint. Keep each change isolated and create a branch only when the user explicitly requests a branch-based workflow:
   - Create a branch for that one change only when the approved workflow requires it, based on the current tip of `refactor/<topic>`.
   ...
   - Record a short result (pass/fail, verification output, PR link) in the current session rather than retaining a full command transcript.
...
- When multiple inventory rows are ready, keep them sequential in the current workspace unless the user explicitly requests an isolated branch workflow.
  - This preserves a human checkpoint between changes and avoids implicit workspace, branch, or PR creation.
...
- Automatic multi-stage orchestration can execute several dependent changes without a human checkpoint.
- Keep staged refactors sequential and review each change before starting the next one.
- For a single mechanical sweep, run one command in the current workspace and verify it with the build/vet/test loop.
