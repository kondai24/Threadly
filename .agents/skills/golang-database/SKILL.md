...
- **Write mode** — generating new repository functions, query helpers, or transaction wrappers: follow the skill's sequential instructions and inspect existing query patterns and naming conventions in the current workspace before generating new code.
- **Review/debug mode** — auditing or debugging existing database code: scan for missing `rows.Close()`, un-parameterized queries, missing context propagation, and absent error checks while reading the business logic.
