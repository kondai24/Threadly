  ...
  openclaw:
    ...
    install:
      - kind: go
        ...
        bins: [benchstat]
allowed-tools: Read Edit Write Glob Grep Bash(go:*) Bash(golangci-lint:*) Bash(benchstat:*) Bash(benchdiff:*) Bash(cob:*) Bash(gobenchdata:*) AskUserQuestion
---
...
## Comparing Optimization Variants Safely

When several competing optimization hypotheses exist for the same bottleneck, implement and measure each variant sequentially in the current workspace by default. Do not delegate work or create worktrees automatically; only use an explicitly approved isolated workflow when the user requests it.

**Run the benchmarks serially, not concurrently.** Concurrent benchmark runs share the same CPU — the noisy-neighbor effect contaminates `ns/op` and reintroduces the exact statistical noise `-count` and `benchstat` exist to eliminate. Compare every variant's `benchstat` output against the **same** baseline report and keep the winner.
